<div class="card bg-white rounded-10 border border-white mb-4">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 p-20">
        <div>
            ...
            <p class="text-body fs-14 mb-0">Daftar permintaan pemeriksaan lab dan status hasil pasien.</p>
        </div>
    </div>
    <div class="p-20 border-top">
        <form class="row g-2 align-items-center">
            <input type="hidden" name="page" value="simrs-laboratory">
            ...
            <div class="col-md-2"><a class="btn btn-light w-100 erp-btn" href="<?= url('simrs-laboratory') ?>">Reset</a></div>
        </form>
    </div>
            ...
            <table class="table align-middle">
                ...
                <?php foreach ($rows as $row): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($row['tests']) ?></td>
                        <td><?= htmlspecialchars($row['order_date']) ?></td>
                        <td><?= simrsStatusBadge($row['status'] ?? '', 'order') ?></td>
                    </tr>
                <?php endforeach; ?>
                <?php if (empty($rows)): ?><tr><td colspan="6" class="text-center py-4">Belum ada order laboratorium.</td></tr><?php endif; ?>
                </tbody>
            </table>
        </div>
        <?= adminListFooter($baseRoute ?? 'simrs-laboratory', $search ?? '', $currentPage ?? 1, $totalPages ?? 1, $totalData ?? count($rows ?? []), $limit ?? 10) ?>
    </div>
</div>
